...
{
    class BinaryMentionTypeClassifier
    {
        ...
        static void Main(string[] args)
        {
            ...
            string str = "";
            StreamWriter sw = new StreamWriter(dir + "mentionTypeClassifier.tsv");
            while((str=sr.ReadLine())!=null)
            {
                string[] toks = str.Split('\t');
            ...
            sr = new StreamReader(dir + "matchArticlesCoreferencedPOSTagged.txt");
            while((str=sr.ReadLine())!=null)
            {
            ...
            //featureNames.Add("Number of Single Event Words");
            //featureNames.Add("Number of Multiple Event Words");
            ...
            foreach (string instance in instance2Label.Keys)
            {
                ...
                //3. event words
                int singleEventWords = 0;
                int multipleEventWords = 0;
                foreach (string mt in mentionToks)
                {
                    if (Global.singularEventWords.Contains(mt.Split('_')[0]))
                        singleEventWords++;
                    if (Global.multipleEventWords.Contains(mt.Split('_')[0]))
                        multipleEventWords++;
                }
                //instance2Features[instance].Add(singleEventWords);
                //instance2Features[instance].Add(multipleEventWords);
                ...
                instance2Features[instance].Add(myList1[0].Value / (myList1[1].Value + 1));
                instance2Features[instance].Add(myList1[0].Value - myList1[1].Value);
                instance2Features[instance].Add(myList2[0].Value);
                instance2Features[instance].Add(myList2[0].Value / (myList2[1].Value + 1));
                instance2Features[instance].Add(myList2[0].Value - myList2[1].Value);
                instance2Features[instance].Add(myList3[0].Value);
                instance2Features[instance].Add(myList3[0].Value / (myList3[1].Value + 1));
                instance2Features[instance].Add(myList3[0].Value - myList3[1].Value);
                ...
                foreach (string s in matchNum2Commentary[match])
                {
                    ...
                    int hasOutGuy=0;
                    int hasBowler=0;
                    string outGuy=player2FullName[toks[25]];
                    foreach(string o in outGuy.Split(' '))
                        if(selection.Contains(o))
                            hasOutGuy=1;
                    string bowler = player2FullName[toks[9]];
            ...
            string dataFile = "mentionTypeClassifier.tsv";
            ProcessStartInfo start = new ProcessStartInfo();
            start.UseShellExecute = false;
            start.RedirectStandardOutput = true;
            start.FileName = @"C:\Users\gmanish\Downloads\TLC_2.4.197.0.Single\TL.exe";
            start.Arguments = dir + dataFile + @" /k=10 /inst=TextInstances{header=+} /cacheinst=- /cl=FastRankBinaryClassification{} /summary=+ /pr=" + dir + @"\mentionTypeClassifier.pr.txt /o=" + dir + @"\mentionTypeClassifier.inst.txt /t=+ /trsess=1";
            StreamWriter outFile = new StreamWriter(dir + "mentionTypeClassifier.out.txt");
            ...
            sr = new StreamReader(dir + "mentionTypeClassifier.inst.txt");
            StreamWriter sw1 = new StreamWriter(dir + "mentionTypeClassifier.wrong.txt");
            StreamWriter sw2 = new StreamWriter(dir + "mentionTypeClassifier.right.txt");
            sr.ReadLine();
            sw1.Write("InstanceID\tTrue\tAssigned\tProbability\tMentionText\tNormalMentionText");
            ...
            while((str=sr.ReadLine())!=null)
            {
                ...
                if (!toks[1].Equals(toks[2]))
                {
                    ...
                    string normalText = "";
                    foreach (string n in instance2MentionText[toks[0]].Split(' '))
                        normalText += n.Split('_')[0] + " ";
                    normalText = normalText.Trim();
                    sw1.Write("\t"+normalText);
                    List<double> features = instance2Features[instance];
                    foreach (double d in features)
                        sw1.Write("\t" + d);
                    sw1.Write("\n");
