...
namespace CricketLinking
{
    class MultiBallClassifier
    {
        ...
        static void Main(string[] args)
        {
            ...
            string str = "";
            StreamWriter sw = new StreamWriter(dir + "multipleMentionClassifier.tsv");
            while ((str = sr.ReadLine()) != null)
            {
                string[] toks = str.Split('\t');
            ...
            featureNames.Add("Number of BAT Words");
            featureNames.Add("Number of BOWL Words");
            featureNames.Add("Number of BATBOWL Words");
            featureNames.Add("Number of WICKETS Words");
            ...
            featureNames.Add("Number of OVERS Words");
            featureNames.Add("Number of OTHERS Words");
            featureNames.Add("Number of POWERPLAY Words");
            featureNames.Add("Number of REFERRAL-DROPPED Words");
...
int numMultiBAT=0;
int numMultiBOWL=0;
int numMultiBATBOWL=0;
int numMultiWICKETS=0;
int numMultiPARTNERSHIP=0;
int numMultiEXTRAS=0;
int numMultiFOUR=0;
int numMultiSIX=0;
int numMultiOVERS=0;
int numMultiOTHERS=0;
int numMultiPOWERPLAY=0;
int numMultiREFERRAL_DROPPED=0;

                foreach (string mt in mentionToks)
                {
                    if (Global.multiEXTRASWords.Contains(mt.Split('_')[0]))
                        numMultiEXTRAS++;
                    if (Global.multiBATWords.Contains(mt.Split('_')[0]))
                        numMultiBAT++;
                    if (Global.multiBOWLWords.Contains(mt.Split('_')[0]))
                        numMultiBOWL++;
                    if (Global.multiBATBOWLWords.Contains(mt.Split('_')[0]))
                        numMultiBATBOWL++;
                    if (Global.multiSIXWords.Contains(mt.Split('_')[0]))
                        numMultiSIX++;
                    if (Global.multiWICKETSWords.Contains(mt.Split('_')[0]))
                        numMultiWICKETS++;
                    if (Global.multiPARTNERSHIPWords.Contains(mt.Split('_')[0]))
                        numMultiPARTNERSHIP++;
                    if (Global.multiFOURWords.Contains(mt.Split('_')[0]))
                        numMultiFOUR++;
                    if (Global.multiOVERSWords.Contains(mt.Split('_')[0]))
                        numMultiOVERS++;
                    if (Global.multiOTHERSWords.Contains(mt.Split('_')[0]))
                        numMultiOTHERS++;
                    if (Global.multiPOWERPLAYWords.Contains(mt.Split('_')[0]))
                        numMultiPOWERPLAY++;
                    if (Global.multiREFERRAL_DROPPEDWords.Contains(mt.Split('_')[0]))
                        numMultiREFERRAL_DROPPED++;
                }

                instance2Features[instance].Add(numMultiBAT);
                instance2Features[instance].Add(numMultiBOWL);
                instance2Features[instance].Add(numMultiBATBOWL);
                instance2Features[instance].Add(numMultiWICKETS);
                instance2Features[instance].Add(numMultiPARTNERSHIP);
                instance2Features[instance].Add(numMultiEXTRAS);
                instance2Features[instance].Add(numMultiFOUR);
                instance2Features[instance].Add(numMultiSIX);
                instance2Features[instance].Add(numMultiOVERS);
                instance2Features[instance].Add(numMultiOTHERS);
                instance2Features[instance].Add(numMultiPOWERPLAY);
                instance2Features[instance].Add(numMultiREFERRAL_DROPPED);
            ...
            foreach (string instance in instance2Features.Keys)
            {
                ...
                for (int i = 0; i < Global.multipleClasses.Count(); i++)
                    if (label.Equals(Global.multipleClasses[i]))
                        classLabel = i;
            ...
            string dataFile = "multipleMentionClassifier.tsv";
            ProcessStartInfo start = new ProcessStartInfo();
            start.UseShellExecute = false;
            start.RedirectStandardOutput = true;
            start.FileName = @"C:\Users\gmanish\Downloads\TLC_2.4.197.0.Single\TL.exe";
            start.Arguments = dir + dataFile + @" /k=3 /inst=TextInstances{header=+} /cacheinst=- /cl=OVA{p=FastRankBinaryClassification} /summary=+ /pr=" + dir + @"\multipleMentionClassifier.pr.txt /o=" + dir + @"\multipleMentionClassifier.inst.txt /t=+ /trsess=1";
            StreamWriter outFile = new StreamWriter(dir + "multipleMentionClassifier.out.txt");
            ...
            StreamWriter sw1 = new StreamWriter(dir + "multipleMentionClassifier.wrong.txt");
            StreamWriter sw2 = new StreamWriter(dir + "multipleMentionClassifier.right.txt");
            sr.ReadLine();
